...
def repo2cwl_function(params):
    ...
    if not os.path.exists(cwl_dir):
        os.mkdir(cwl_dir)
    exit_code = repo2cwl(["https://github.com/mosoriob/MIC_model", "-o", cwl_dir])
    ...
    response =  {"image_name": image_name, 'specs': specs_s3}
    return response
...
def login_push_function(task_instance, **kwargs):
    ...
    return {'name': new_image_name, 'tag': tag, 'specs': specs}
